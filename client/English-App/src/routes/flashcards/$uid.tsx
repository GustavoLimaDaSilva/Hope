...
export const Route = createFileRoute('/flashcards/$uid')({
...
export default function FlashcardsIndex() {
    ...
    return (
        <>
            ...
            {decksData.lessonDecksData.map((d, index: number) => <Link to={`/flashcards/decks/${d.id}`} key={index}>{d.name}</Link>)}
            <h2>Seus decks personalizados</h2>
            {decksData.personalDecksData.map((d, index: number) => <Link to={`/flashcards/decks/${uid}/${d.id}`} key={index}>{d.name}</Link>)}
            <div>
